				...
				<form id="manage-purchase_order">
					...
					<div class="row">
						<div class="form-group col-sm-6">
							...
							<input type="text" min='1' class="form-control form-control-sm " id="po_code" name="po_code" value="<?php echo isset($po_code) ? $po_code : '' ?>" readonly >
							<small>This field will be autogenerated</small>
...
<script>
	...
	$(function(){
		$('#print').click(function(){
			start_load();
			var nw = window.open("print_po.php?id=<?php echo isset($id) ? $id : '' ?>","","height=900,width=1200,left=75,top=25")
		...
		if('<?php echo isset($status) && $status != 0  ?>' == 1){
			$('#p-form').remove()
			$('#manage-purchase_order').find('input,textarea, button,select').attr('disabled',true)
	...
	$('.number').on('input',function(){
 	...
 	$('#manage-purchase_order').submit(function(e){
 		e.preventDefault()
 		start_load()
         $('#po_code').removeClass('border-danger')
         $('.err').remove()
 		$.ajax({
 				...
 				if(resp == 1){
 					...
 					setTimeout(function(){
 						if('<?php echo isset($_GET['id']) ?>' == 1)
 							...
 							location.replace('index.php?page=purchase_order')
 					},500)
 				}else if(resp == 2){
                    $('#po_code').addClass('border-danger').trigger('focus')
                    $('#po_code').after('<span class="err text-danger"><small>PO Code Already exists.</small></span>')
                }else{
